...
bool Target::setup() {
  ...
  // Only add when there are user-supplied arguments: a bare -Wl, flag with no
  // input files causes clang to enter link mode and invoke ld, which then
  // fails with "undefined symbols for _main".
  if (!args.empty()) {
